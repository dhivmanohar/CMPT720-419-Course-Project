...
class Engine(gym.Env, gym.utils.EzPickle):
    ...
    DEFAULT = {
        ...
        'pillars_dist_cost':1.0,  # Cost (per step) for being within distance threshold
    ...
    def obs_lidar_natural(self, group):
        ...
        angles = np.zeros(self.lidar_num_bins)
        ...
        for i in range(self.lidar_num_bins):
            ...
            if dist >= 0:
                obs[i] = np.exp(-dist)
                angles[i] = theta
                if  is_obstacle and obs[i] > min_distance:
                    min_distance = obs[i]
                    self.dmin = min_distance
        ...
        if group == GROUP_GOAL:
            if np.all(obs == np.zeros(self.lidar_num_bins)):
                ...
                z = np.complex(*self.ego_xy(random_pos[:2]))
                self.goal_angles = np.array([np.angle(z) % (np.pi * 2)])
                self.goal_min_dist = np.array([self.distance_to_random_goal])
                #self.goal_angles = np.zeros(self.lidar_num_bins)
            else:
                self.goal_not_seen = False
                #idx = np.where(obs!=0)[0]
                self.goal_min_dist = np.amin(obs)
                self.goal_angles = angles
                #self.goal_angles = np.split(angles[idx],np.where(np.diff(idx)!=1)[0]+1)

        if group == GROUP_PILLAR:
            if np.any(obs != np.zeros(self.lidar_num_bins)):
                self.pillar_in_view = True
                #idx = np.where(obs!=0)[0]
                self.pillar_distances = obs
                self.pillar_angles = angles
                #self.pillar_distances = np.split(obs[idx],np.where(np.diff(idx)!=1)[0]+1)
                #self.pillar_angles = np.split(angles[idx],np.where(np.diff(idx)!=1)[0]+1)
            else:
                self.pillar_in_view = False
                self.pillar_distances = np.zeros(self.lidar_num_bins)
                self.pillar_angles = np.zeros(self.lidar_num_bins)

        if group == GROUP_GREMLIN:
            if np.any(obs != np.zeros(self.lidar_num_bins)):
                self.gremlin_in_view = True                
                #idx = np.where(obs!=0)[0]
                self.gremlin_distances = obs
                self.gremlin_angles = angles
                #self.gremlin_distances = np.split(obs[idx],np.where(np.diff(idx)!=1)[0]+1)
                #self.gremlin_angles = np.split(angles[idx],np.where(np.diff(idx)!=1)[0]+1)
            else:
                self.gremlin_in_view = False  
                self.gremlin_distances = np.zeros(self.lidar_num_bins)
                self.gremlin_angles = np.zeros(self.lidar_num_bins)                
        return obs
    ...
    def obs_lidar_pseudo(self, positions):
        ...
        '''
        #currently not working with different lidar range (fov + fov offset factor)
        obs = np.zeros(self.lidar_num_bins)
        for pos in positions:
            ...
            fov = self.lidar_fov_factor * np.pi
            starting_fov_angle = self.lidar_fov_offset_factor * np.pi
            angle = np.angle(z) % fov
            bin_size = fov / self.lidar_num_bins
            bin = int(angle / bin_size)
            # bin_angle = bin_size * bin
            bin_angle = starting_fov_angle + (bin_size * bin)
    ...
    def cost(self):
        ...
        # Cost Pillar Avoidance
        # Reward maximizing distance from pillar in view (now for obstacle group) -trpo
        if self.constrain_pillars and self.pillar_in_view:
            cost['cost_pillars_dist'] = 0
            idx = np.where(self.pillar_distances!=0)[0]
            if idx.size == 0:
                pillar_distances_split = [self.pillar_distances]
            else:
                pillar_distances_split = np.split(self.pillar_distances[idx],np.where(np.diff(idx)!=1)[0]+1)
            for i in range(np.shape(pillar_distances_split)[0]):
                max_idx = np.argmax(pillar_distances_split[i])                
                if pillar_distances_split[i][max_idx] >= self.pillar_distance_threshold:
                    cost['cost_pillars_dist'] += np.tanh(pillar_distances_split[i][max_idx] - self.pillar_distance_threshold) * self.pillars_dist_cost
        
        # Reward maximizing distance from pillar in view (now for obstacle group) -trpo
        if self.constrain_gremlins and self.gremlin_in_view:
            cost['cost_gremlins_dist'] = 0
            idx = np.where(self.gremlin_distances!=0)[0]
            if idx.size == 0:
                gremlin_distances_split = [self.gremlin_distances]
            else:
                gremlin_distances_split = np.split(self.gremlin_distances[idx],np.where(np.diff(idx)!=1)[0]+1)
            for i in range(np.shape(gremlin_distances_split)[0]):
                max_idx = np.argmax(gremlin_distances_split[i])                
                if gremlin_distances_split[i][max_idx] >= self.gremlins_dist_threshold:  
                    cost['cost_gremlins_dist'] += np.tanh(gremlin_distances_split[i][max_idx]-self.gremlins_dist_threshold) * self.gremlins_dist_cost
        
        if self.constrain_pillars or self.constrain_gremlins:
            cost['cost_angle_gap'] = 0            
            if self.pillar_in_view or self.gremlin_in_view:                
                if (np.amax(self.gremlin_distances) >= self.gremlins_dist_threshold) or (np.amax(self.pillar_distances) >= self.pillar_distance_threshold):
                    gap_theta = 0
                    goal_theta = 0
                    alpha = 0.5
                    #idx = np.where(obs!=0)[0]
                    #pillar_angles_split = np.split(self.pillar_angles[idx],np.where(np.diff(idx)!=1)[0]+1)
                    #gremlin_angles_split = np.split(self.gremlin_angles[idx],np.where(np.diff(idx)!=1)[0]+1)
                    cond1 = (self.pillar_angles == 0)
                    cond2 = (self.gremlin_angles == 0)
                    gap_idx = np.where(cond1 & cond2)[0]
                    combo = np.ones(self.lidar_num_bins,dtype=int)
                    combo[gap_idx] = 0

                    groups = [[i for i,_ in g] for key, g in groupby(enumerate(combo.tolist()), key=itemgetter(1)) if not key]            
                    group = max(groups, key=len, default=[-1, -1])
                    if group[0] != -1:
                        if group[0]!= group[-1]:
                            min_theta = (self.lidar_fov_offset_factor * np.pi) + ((group[0]/ self.lidar_num_bins) * np.pi * self.lidar_fov_factor)
                            max_theta = (self.lidar_fov_offset_factor * np.pi) + ((group[-1]/ self.lidar_num_bins) * np.pi * self.lidar_fov_factor)
                            gap_theta = max_theta - min_theta
                        else:
                            gap_theta = 1/2*((self.lidar_fov_offset_factor * np.pi) + ((group[0]/ self.lidar_num_bins) * np.pi * self.lidar_fov_factor))
                    else:
                        gap_theta = 0

                    
                    groups = [[i for i,_ in g] for key, g in groupby(enumerate(self.goal_angles.tolist()), key=itemgetter(1)) if key]     
                    goal_angles_idx = max(groups, key=len, default=[-1, -1])
                    if goal_angles_idx[0] != goal_angles_idx[-1]:
                        min_theta = (self.lidar_fov_offset_factor * np.pi) + ((goal_angles_idx[0]/ self.lidar_num_bins) * np.pi * self.lidar_fov_factor)
                        max_theta = (self.lidar_fov_offset_factor * np.pi) + ((goal_angles_idx[-1]/ self.lidar_num_bins) * np.pi * self.lidar_fov_factor) 
                        goal_theta = max_theta - min_theta
                    else:
                        goal_theta = (self.lidar_fov_offset_factor * np.pi) + ((goal_angles_idx[0]/ self.lidar_num_bins) * np.pi * self.lidar_fov_factor)
                

                    guided_theta = (alpha*self.dmin)*gap_theta + goal_theta/((alpha*self.goal_min_dist)+1)
                    cost['cost_angle_gap'] = (guided_theta / (np.pi/2))
        #if self.
    ...
    def reward(self):
        ...
        # New reward for exploration - trpo
    ...
    def render_lidar(self, poses, color, offset, group):
        ...
        for i, sensor in enumerate(lidar):
            ...
            theta = starting_fov_angle + (fov * i / self.lidar_num_bins)
